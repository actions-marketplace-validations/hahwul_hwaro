...
#
# These tests spin up a full build pipeline (Builder#run) against temporary
# project directories and verify the rendered output.  They are the most
# important safety-net for catching regressions that break real sites.
# =============================================================================

# ---------------------------------------------------------------------------
# Helper: set up a minimal project skeleton in a temp directory and build it.
# Yields the output_dir path so the caller can make assertions on files.
# ---------------------------------------------------------------------------
private def build_site(
  ...
  Dir.mktmpdir do |dir|
    Dir.cd(dir) do
      ...
      # Register all content hooks (markdown, SEO, taxonomy) — same as
      # BuildCommand and Server do.  Without this, taxonomy pages won't be
      # generated and the lifecycle-based parsing/transform won't fire.
